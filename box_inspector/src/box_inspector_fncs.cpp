...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
        ...
        geometry_msgs::PoseStamped shipment_poses_wrt_world;
        int num_products;

        num_products = shipment_wrt_box.products.size();
        ...
        cam_pose = box_pose_wrt_world.pose;

        for (int i = 0; i < num_products; i++) {
            product = shipment_wrt_box.products[i];
            shipment_poses_wrt_world = compute_stPose(cam_pose, product.pose);
            product.pose = shipment_poses_wrt_world.pose;
